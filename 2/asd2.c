...
int main(void){
    ...
    for(size_t i = 0; i < gs.count; ++i){
        printf("id: %d ", gs.items[i].game_id);
        bool possible = true;
        ...
        for(size_t j=0; j < gs.items[i].count; ++j){
            ...
            if(max_r < tmp.red)   max_r = tmp.red;
            if(max_g < tmp.green) max_g = tmp.green;
            if(max_b < tmp.blue)  max_b = tmp.blue;

        }

        if(possible) id_sum += gs.items[i].game_id;
        
        int set_pow = max_r*max_g*max_b;
        set_pow_sum+=set_pow;
        printf("red %d green %d blue %d pow %d\n",max_r,max_g,max_b, set_pow);
    }
    printf("  id_sum: %d  set_pow_sum: %d\n", id_sum, set_pow_sum);
