...
function calc_area(coords) 
    ...
    for i=1,(#coords-1) do
        local x1 = coords[i][1]
        local y1 = coords[i][2]
        local x2 = coords[i+1][1]
        local y2 = coords[i+1][2]
        area = area + (x1*y2)-(x2*y1)
    end

    area = area + (coords[#coords][1] * coords[1][2]) - (coords[1][1] * coords[#coords][2])
    area = area//2
...
function traverse(start_idx, map_bytes, W,H,L)
    ...
    repeat 
        ...
        idx = (x+y*W)
        -- write `#` to positions visited
        map_bytes[prev_idx]=string.byte('#')
        -- save pipe coords
        table.insert(pipe_coords, {x,y})
    ...
    --
    local chars = {}
    for _,b in ipairs(map_bytes) do
        table.insert(chars,string.char(b))
    end
    local str = table.concat(chars)
    print(str)
    --for i,c in ipairs(pipe_coords) do 
    --    print(i,"x",c[1],"y",c[2])
    --end
    local area = calc_area(pipe_coords)
    print("enclosed area: ",area + 1 - ((loop_len+1)//2))
end
